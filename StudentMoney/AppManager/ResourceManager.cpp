...
vector<pair<string, pair<int, pair<string, int>>>> ResourceManager::readRecordFile(string data)
{
	string s;
	vector<string> v;
	ifstream fin("../Data/FileData/" + data, ios::in | ios::app);
	while (getline(fin, s)) {
		v.push_back(s);
	}
	fin.close();
	...
	for (int i = 0; i < (int)v.size(); i += 4) {
		string name = v[i];
		int coin = 0;
		if (v[i+1] != "" ) coin = stoi(v[i + 1]);
		string note = v[i + 2];
		int day = 1;
		if (v[i+3] != "") day = stoi(v[i + 3]);
		users.push_back({ name, {coin, {note,day} } });
	}
	return users;
}
